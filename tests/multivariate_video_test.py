...
def _generate_motion_capture(joints_to_use, prefix=None, add_xyz=True):
    ...
    dists, candidates, dims, elbow_points = ml.fit_k_elbow(
        k_max,
    ...
    # k_max,np.arange(80, 150, 10), subsample=1)
    ...
    # dists, candidates, elbow_points = ml.fit_dims_elbow(
    #     13,
    #     plot_elbows=True,
    #     motif_length=motif_length)
    ...
    out_path = 'video/motiflet_' + amc_name + '_full.gif'
    FuncAnimation(fig,
                lambda i: draw_frame(ax, motions, joints, i,
                                    joints_to_highlight=filtered_joints),
                range(0, series.shape[1], 4)).save(
    out_path,
    bitrate=3000,
    fps=120)
