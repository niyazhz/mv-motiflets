...
def get_pairwise_extent(D_full, motifset_pos, dims, upperbound=np.inf):
    ...
    for ii in range(len(motifset_pos) - 1):
        ...
        for jj in range(ii + 1, len(motifset_pos)):
            ...
            motifset_extent = max(motifset_extent, D_full[dims, i, j].mean())
...
def get_approximate_k_motiflet(
        ts, m, k, D, knns, best_dims, upper_bound=np.inf
):
    ...
    for order in np.arange(n):
        for d in best_dims[order]:
            dims = best_dims[order]
...
def search_k_motiflets_elbow(
    ...
    for test_k in tqdm(range(k_max_, 1, -1),
        ...
        best_dims = compute_relevant_dimensions(D_full, knns, test_k, use_dim)

        candidate, candidate_dist, candidate_dims = get_approximate_k_motiflet(
            data_raw, m, test_k, D_full, knns, best_dims,
...
def search_k_motiflets_n_dims_elbow(
        k,
        data,
        motif_length,
        n_dims=None,
    ...
    # non-overlapping motifs only
    k_motiflet_distances = np.zeros(use_dim + 1)
    k_motiflet_candidates = np.empty(use_dim + 1, dtype=object)
    k_motiflet_dims = np.empty(use_dim + 1, dtype=object)

    upper_bound = np.inf
    for dims in tqdm(range(use_dim, 0, -1),
                    desc='Compute dimensions (' + str(use_dim) + ")",
                    position=0, leave=False):

        best_dims = compute_relevant_dimensions(D_full, knns, k, dims)

        candidate, candidate_dist, candidate_dims = get_approximate_k_motiflet(
            data_raw, m, k, D_full, knns, best_dims,
            upper_bound=upper_bound,
        )
        k_motiflet_distances[dims] = candidate_dist
        k_motiflet_candidates[dims] = candidate

        # FIXME: what were the actual best dimensions used?
        k_motiflet_dims[dims] = candidate_dims

        # upper_bound = min(candidate_dist, upper_bound)

        # # compute a new upper bound
        # if candidate is not None:
        #     dist_new = get_pairwise_extent(D_full, candidate[:dims], candidate_dims)
        #     upper_bound = min(upper_bound, dist_new)
