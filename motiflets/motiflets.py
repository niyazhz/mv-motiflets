...
def read_dataset_with_index(dataset, gt_file_name, sampling_factor=10000):
    ...
    gt = read_ground_truth(gt_file_name)
    if gt is not None:
        if factor > 1:
            for column in gt:
...
def compute_distance_matrix(time_series,
    ...
    """ Compute the full Distance Matrix between all pairs of subsequences of a
        ...
        This implementation is in O(d n^2) by using the sliding dot-product.

        Details are given within the paper Section 4.2 Distance Matrices Computation.
        ...
        sum_dims : bool
            Sum distances overa all dimensions into one row for
...
@njit(fastmath=True, cache=True)
def get_pairwise_extent(D, motifset_pos, dims, upperbound=np.inf):
    """Computes the extent of the motifset.

    Details are given within the paper Section 2.2 Distance Measure Definitions.
    ...
    motifset_pos : array-like
        The motif set start-offsets
    dims : array-like
        The dimensions used for the motif set
    ...
    for ii in range(len(motifset_pos) - 1):
        ...
        for jj in range(ii + 1, len(motifset_pos)):
            ...
            motifset_extent = max(motifset_extent, D[dims, i, j].mean())
...
def _argknn(
    ...
    Details are given within the paper Section 4.3 k-Nearest-Neighbors (k-NN) Computation.
...
def get_approximate_k_motiflet(
        ts, m, k, D, knns, relevant_dims, upper_bound=np.inf
):
    """Compute the approximate k-Motiflets.

    Details are given within the paper Section 4.1 Overview of the Algorithm Development.
    ...
    D : 3d array-like
        The distance matrix
    knns : 3d array-like
        The k nearest neighbors matrix
    relevant_dims : 2d array-like
        The best dimensions for each subsequence
    upper_bound : float
        Used for admissible pruning
    ...
    Tuple
        ...
        motiflet_dims:
            The dimensions of the motiflet found
    """
    n = ts.shape[-1] - m + 1
    ...
    for d in range(0, D.shape[0]):
        np.fill_diagonal(D[d], 0)

    # order by increasing k-nn distance
    for order in np.arange(n):
        for d in relevant_dims[order]:
            dims = relevant_dims[order]
            knn_idx = knns[d, order]
            if len(knn_idx) >= k and knn_idx[k - 1] >= 0:
                if D[dims, order, knn_idx[k - 1]].mean() <= motiflet_dist:
                    # get_pairwise_extent() requires the full distance matrix
                    motiflet_extent = get_pairwise_extent(D, knn_idx[:k], dims, motiflet_dist)
...
def _filter_unique(elbow_points, candidates, motif_length):
    ...
    for i in range(len(elbow_points)):
        ...
        for j in range(i + 1, len(elbow_points)):
            if candidates[elbow_points[i]] is not None and candidates[elbow_points[j]] is not None:
...
def find_au_ef_motif_length(
    ...
    Details are given within the paper Section 3.3.3 Parameter Selection.
...
def search_k_motiflets_elbow(
    ...
    for test_k in tqdm(range(k_max_, 1, -1),
        ...
        relevant_dims = compute_relevant_dimensions(D_full, knns, test_k, use_dim)

        #if test_k == 13:
        upper_bound = np.inf

        candidate, candidate_dist, candidate_dims = get_approximate_k_motiflet(
            data_raw, m, test_k, D_full, knns, relevant_dims,
        ...
        if candidate is not None:
            dist_new = get_pairwise_extent(D_full, candidate[:test_k], candidate_dims)
...
def compute_relevant_dimensions(D, knns, k, use_dim):
    """Computes the relevant dimensions.

    Identifies the most relevant dimensions for each subsequence
    based on the distance matrix and k-nearest neighbors.

    Details are given within the bachelorthesis Section 4.4 Dimension Selection.
    ...
    Tuple
        relevant_dims : 2d array-like
            The use_dim most relevant dimensions for each subsequence
    """

    last_knns = knns[:,:,k-1]
    last_knns_distance = np.take_along_axis(D, last_knns[:,:,None], axis=2)
    argsort_last_knns = np.argsort(last_knns_distance, axis=0)[:use_dim]

    relevant_dims = argsort_last_knns.transpose(1, 0, 2).reshape(argsort_last_knns.shape[1], use_dim)
...
def runtime_test(
    ...
    """ Computes multiple k-Motiflets for a given k for runtime testing.

    Details are given within the bachelorthesis Section 5.5 Runtime.
    ...
    relevant_dims = compute_relevant_dimensions(D_full, knns, k, use_dim)

    candidate, candidate_dist, candidate_dims = get_approximate_k_motiflet(
        data_raw, m, k, D_full, knns, relevant_dims,
        upper_bound=upper_bound,
    )
