...
class Motiflets:
    ...
    def fit_k_elbow(
        ...
        return self.dists, self.motiflets, self.motiflets_dims, self.elbow_points
    ...
    def fit_dims_elbow(
        ...
        if n_dims is None and self.n_dims is None:
            n_dims = self.series.shape[0]
        elif n_dims is None and self.n_dims is not None:
            n_dims = self.n_dims
        else:
            self.n_dims = n_dims

        self.dists, self.motiflets, self.motiflets_dims, self.elbow_points = plot_dims_elbow(
            k,
            self.series,
            n_dims=n_dims,
            ds_name=self.ds_name,
...
def plot_motifsets(
    ...
    for dim in range(data_raw.shape[0]):
        ...
        if motifsets is not None:
            #motiflet_plot_order = np.array([len(mtf) for mtf in motifsets])[::-1].argsort() + data_raw.shape[0]
...
def _plot_elbow_points(
    ...
    if plot_dims_elbow:
        ax.set(xlabel='Size (dims)', ylabel='Extent')
    else:
        ...
        if motiflet is not None:
            ...
            for dim in range(data_raw.shape[0]):
                if (motifset_candidates_dims is None or
                        dim == motifset_candidates_dims[elbow_points][0][1]): #dim == motifset_candidates_dims[elbow_points][0][1]):
                    pos = motiflet[0]
...
def plot_dims_elbow(k_max,
    ...
    startTime = time.perf_counter()
    dists, candidates, candidate_dims, elbow_points, m = ml.search_k_motiflets_n_dims_elbow(
        k_max,
        raw_data,
        motif_length,
        n_dims=n_dims,
    ...
    if plot_elbows:
        _plot_elbow_points(
            ds_name, data, motif_length, elbow_points,
            candidates, dists, motifset_candidates_dims=candidate_dims, plot_dims_elbow=True)
...
def plot_grid_motiflets(
    ...
    #motiflet_plot_order = np.array([len(mtf) for mtf in motiflets])[::-1].argsort() + data_raw.shape[0]
    # motiflet_plot_color = np.array([len(mtf) for mtf in motiflets])[::-1].argsort() + 1
    # new_color_palette = [color_palette[c] for c in motiflet_plot_color].insert(0, color_palette[0])
...
def plot_all_competitors(
    ...
    plot_grid_motiflets(
        ds_name, data, motifsets, elbow_points=elbow_points,
        dists=dists, motif_length=motif_length, show_elbows=False,
        font_size=26,
        ground_truth=ground_truth,
        dimension_labels=dimension_labels,
...
def plot_competitor_motifsets(
    ...
    tick_offsets = []
    axes[0, 0].set_title(ds_name, fontsize=20)
    axes[0, 0].set_xlabel("Seconds", fontsize=12)
    ...
    if motifsets is not None:
        ...
        #axes[1, 0].set_yticks(-np.arange(len(motifsets)) + 1.5)
        axes[1, 0].set_yticks(-np.arange(len(motiflet_names)) + 0.5)
        axes[1, 0].set_yticklabels(motiflet_names, fontsize=12)
        axes[1, 0].set_ylim([-abs(len(motifsets)) + 1, 1])
        axes[1, 0].set_xlim(axes[0, 0].get_xlim())
        axes[1, 0].set_title("Position of Motifsets", fontsize=20)
...
def plot_grid_motiflets_niyaz(
    ...
    ax_bars.set_title("(b) Position of Top Motif Sets", pad=30)
    ...
    if show_elbows:
        ...
        ax_title.set_title('(c) Shape of Top Motif Sets by Method', pad=70)
    ...
    for i, motiflet in enumerate(motiflets):
        if motiflet is not None:
            ...
            if plot_minature: 
                # if ground_truth is not None and i == 0: 
                #     continue
                df_melt = pd.melt(df, id_vars=["time"])
                _ = sns.lineplot(ax=ax_motiflet,
                                 data=df_melt,
                                 x="time", y="value",
                                 label=minature_labels[i],
                                 errorbar=('ci', 99), n_boot=10,
                                 color=color_palette[1 + i],
                                 )
                ax_motiflet.set_ylabel("")
                ax_motiflet.set_title(method_names[i])#, fontsize=title_font_size)

                if isinstance(data, pd.Series):
                    ax_motiflet.set_xlabel(data.index.name)

                sns.despine()
                ax_motiflet.legend(fontsize=legend_font_size-3, loc='upper left').set_visible(True)

    if dimension_labels is not None:
        ax_ts.set_yticks(tick_offsets)
        ax_ts.set_yticklabels(dimension_labels)

    ax_bars.set_yticks(-np.arange(len(y_labels)) + 0.5, )
    ax_bars.set_yticklabels(y_labels, fontsize=font_size-10)
    ax_bars.set_ylim([-len(motiflets) + 1, 1])

    if ground_truth is not None and len(ground_truth) > 0:
        ax_ts.legend(loc="upper left", ncol=label_cols, fontsize=legend_font_size)

    # ax_ts.xaxis.label.set_size(axis_font_size)
    # ax_ts.yaxis.label.set_size(axis_font_size)

    # ax_bars.xaxis.label.set_size(axis_font_size)
    # ax_bars.yaxis.label.set_size(axis_font_size)

    # ax_motiflet.xaxis.label.set_size(axis_font_size)
    # ax_motiflet.yaxis.label.set_size(axis_font_size)

    # ax_ts.title.set_size(title_font_size)
    # ax_bars.title.set_size(title_font_size)
    # ax_motiflet.title.set_size(title_font_size)
